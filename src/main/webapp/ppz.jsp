...
<div id="page">
    <div id="header">
        <div class="top">
            ...
            <a class="version new" href="http://wiki.ued.taobao.net/doku.php?id=user:zhangting:tools:ucool-pro:history:start" target="_blank" title="what's new?">ucool-pro version：0.9.2 <i>?</i></a>
        </div>
    </div>
    <div id="content">
...
//                        DOM.hide('#J_BoxSwitch');
                    ...
                    KISSY.use('ua,overlay, template', function(S, UA, O, T) {
                        ...
                        var mappingChecksLiTemplate = T('{{#each mappings}}<li>'+
                                '<a class="icon-del" href="#" title="删除">删除</a><input type="checkbox" checked="checked" value="{{_ks_value.path}}" {{#if _ks_value.use}}checked{{/if}}/><label for="">{{_ks_value.path}}</label></li>{{/each}}');
                        // 添加的li模板
